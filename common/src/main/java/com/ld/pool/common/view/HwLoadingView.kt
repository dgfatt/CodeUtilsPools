 ...
 * 仿华为加载框
 */
open class HwLoadingView(
    context: Context, @Nullable attrs: AttributeSet?,
    defStyleAttr: Int
) :
    View(context, attrs, defStyleAttr) {
    private val mWholeCircleRadius =
        FloatArray(CIRCLE_COUNT) //记录所有小圆半径
    private val mWholeCircleColors =
        IntArray(CIRCLE_COUNT) //记录所有小圆颜色
    ...
    private fun initAttrs(context: Context, attrs: AttributeSet?) {
        val ta: TypedArray = context.obtainStyledAttributes(attrs, R.styleable.HwLoadingView)
        mSize = ta.getDimension(
            R.styleable.HwLoadingView_hlv_size,
            dp2px(context, 100f).toFloat()
        ).toInt()
        setSize(mSize)
    ...
    override fun onDraw(canvas: Canvas) {
        if (mSize > 0) { //每隔DEGREE_PER_CIRCLE * mAnimateValue角度，绘制所有小圆
            canvas.rotate(
                (DEGREE_PER_CIRCLE * mAnimateValue).toFloat(),
                (mSize / 2).toFloat(),
                (mSize / 2).toFloat()
            )
            for (i in 0 until CIRCLE_COUNT) { //设置小圆颜色
                mPaint!!.alpha = mWholeCircleColors[i]
                //每隔DEGREE_PER_CIRCLE角度，绘制一个小圆
                canvas.drawCircle((mSize / 2).toFloat(), mMaxCircleRadius, mWholeCircleRadius[i], mPaint!!)
                canvas.rotate(DEGREE_PER_CIRCLE.toFloat(), (mSize / 2).toFloat(),
                    (mSize / 2).toFloat()
                )
            }
        }
    ...
    companion object {
        ...
        private const val DEGREE_PER_CIRCLE =
            360 / CIRCLE_COUNT //小圆圆心之间间隔角度差
    }
